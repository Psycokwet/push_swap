...
testing() {
	...
	if [ $nb_char -ne 0 ]; then
		...
		result_checker_perso=$(printf "$result_push_swap\n" | ./checker_scarboni $str);
	else
		result_checker=$(printf "$result_push_swap" | ./checker_linux $str);
		result_checker_perso=$(printf "$result_push_swap" | ./checker_scarboni $str);
	fi
	...
	printf "\n"
	printf "\e[33mchecker perso output\e[0m: \"$result_checker_perso\" ";
	if [ "$result_checker_perso" = "OK" ]; then
		printf "✅";
	elif [ "$result_checker_perso" = "KO" ]; then
		printf "❌";
	else
		printf "❔";
	fi
...
check_programs() {
	if ! command -v ./push_swap &> /dev/null; then
	    ...
	    echo "./checker_linux could not be found" >&2; exit 1;
	fi
}
